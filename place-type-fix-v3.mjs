...
if(!window.__PV_PLACE_TYPE_FIX_V3__){
 ...
 function ensureMetric(label){let m=typeMetric();if(m){m.style.display='';const b=m.querySelector('b');if(b)b.textContent=label;return}const grid=$('#infoBody .info-grid');if(!grid)return;m=document.createElement('div');m.className='info-metric pv-type-metric';m.innerHTML=`<small>${lang()==='it'?'Tipo':'Type'}</small><b></b>`;m.querySelector('b').textContent=label;grid.prepend(m)}
 function apply(kind,tags={},allowGeneric=false){const t=tax();if(!t)return;const real=t.clean(kind||t.classify(tags)),generic=t.isGeneric(real);if(generic&&!allowGeneric){typeMetric()?.remove();const s=$('#infoBody .pv-place-summary span');if(s&&/^(luogo|place|punto di interesse|point of interest)$/i.test(s.textContent.trim()))s.textContent='';return}const label=t.label(real,tags);ensureMetric(label);const s=$('#infoBody .pv-place-summary span');if(s)s.textContent=label;const k=$('#infoBody .pv-place-summary2 .kind');if(k)k.textContent=label;const hidden=$('#infoBody .pv-place-summary b');if(hidden)hidden.textContent=real;
  const active=window.__PV_ACTIVE_PLACE__;const title=$('#infoTitle');if(title&&active&&(!active.tags?.name)&&[active.name,currentRaw(),kind].some(v=>String(v||'').toLowerCase()===String(title.textContent||'').toLowerCase()))title.textContent=label
 }
