...
if(!window.__PV_UNIVERSAL_POI_V1__){
 ...
 let map=null,canvas=null,down=null,lastTap=0,styleTimer=null;
 ...
 function bestFeature(point){
   let fs=[];try{fs=map.queryRenderedFeatures([[point.x-28,point.y-28],[point.x+28,point.y+28]])||[]}catch{return null}
   let best=null,bestScore=95;for(const f of fs){const s=score(f);if(s>bestScore){best=f;bestScore=s}}return best;
 }
 ...
 function openInstant(f,point){
   ...
   $('#infoKicker').textContent=lang()==='it'?'DETTAGLI DEL LUOGO':'PLACE DETAILS';
 ...
 function improveLabels(){
   if(!map?.isStyleLoaded?.())return;const dark=satelliteOn();
   for(const l of map.getStyle()?.layers||[]){
     if(l.type!=='symbol'||l.id.startsWith('pv-poi-'))continue;
     const sl=String(l['source-layer']||'').toLowerCase(),id=String(l.id||'').toLowerCase();
     if(!(sl==='poi'||/poi|amenity|shop|aerodrome|place.*label|transit|station/i.test(id+' '+sl)))continue;
     try{map.setPaintProperty(l.id,'text-opacity',1)}catch{}
     try{map.setPaintProperty(l.id,'icon-opacity',1)}catch{}
     try{map.setPaintProperty(l.id,'text-color',dark?'#ffffff':'#3f4858')}catch{}
     try{map.setPaintProperty(l.id,'text-halo-color',dark?'rgba(0,0,0,.88)':'rgba(255,255,255,.96)')}catch{}
     try{map.setPaintProperty(l.id,'text-halo-width',dark?1.8:1.5)}catch{}
     try{map.setPaintProperty(l.id,'text-halo-blur',.2)}catch{}
   }
 }
 function bind(){
   document.addEventListener('pointerdown',e=>{if(e.target===canvas)down={x:e.clientX,y:e.clientY,t:Date.now()}},true);
   document.addEventListener('pointerup',e=>{
     if(e.target!==canvas||!down)return;const d=down;down=null;const moved=Math.hypot(e.clientX-d.x,e.clientY-d.y),elapsed=Date.now()-d.t;if(moved>12||elapsed>700)return;
     const rect=canvas.getBoundingClientRect(),point={x:e.clientX-rect.left,y:e.clientY-rect.top},f=bestFeature(point);if(!f)return;
     const id=layerId(f);if(id.startsWith('pv-poi-')||id==='quakes'||id==='quake-glow')return;
     if(Date.now()-lastTap<250)return;lastTap=Date.now();window.__PV_UNIVERSAL_POI_UNTIL=Date.now()+1100;
     e.preventDefault();e.stopImmediatePropagation();openInstant(f,point);
   },true);
 ...
 async function init(){for(let i=0;i<180;i++){if(window.__PV_MAP__?.getCanvas){map=window.__PV_MAP__;canvas=map.getCanvas();break}await sleep(50)}if(!map||!canvas)return;bind();setTimeout(improveLabels,100);map.on('styledata',()=>{clearTimeout(styleTimer);styleTimer=setTimeout(improveLabels,120)});map.on('idle',()=>{clearTimeout(styleTimer);styleTimer=setTimeout(improveLabels,80)})}
 init();
}
