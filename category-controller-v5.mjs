...
if(!window.__PV_CATEGORY_V5__){
 ...
 async function select(id,user=true){
   ...
   const c=cached(id,b);if(c){render(c.rows,id);status(`${c.count} ${tr().results} · ${label(id)}`,!c.rows.length);if(Date.now()-c.time<45000)return}
   ...
   aborter=new AbortController();sync();if(user&&!c)status(`${tr().loading} ${label(id)}`);try{const data=await fetchCategory(id,b,aborter.signal);if(mine!==seq||active!==id)return;const item={box:padBox(b,.04),time:Date.now(),rows:data.rows||[],count:data.count??(data.rows||[]).length};catCache.set(id,item);render(item.rows,id);status(`${item.count} ${tr().results} · ${label(id)}`,!item.rows.length)}catch(e){if(e.name!=='AbortError'&&mine===seq&&!c)status(tr().none,true)}finally{if(mine===seq){aborter=null;sync()}}
 }
