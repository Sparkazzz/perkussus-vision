...
if(!window.__PV_CATEGORY_V5__){
 ...
 function status(msg,error=false){const e=$('#pvMapSearchStatus');if(!e)return;e.textContent=msg;e.classList.toggle('error',error);e.classList.add('show');clearTimeout(status.t);status.t=setTimeout(()=>e.classList.remove('show'),1800)}
 ...
 function padBox(b,p=.12){const dy=(b.north-b.south)*p,dx=(b.east-b.west)*p;return{south:round(b.south-dy),west:round(b.west-dx),north:round(b.north+dy),east:round(b.east+dx)}}
 ...
 function render(list,id){
   remove();rows=list||[];if(!rows.length)return;
   const features=rows.map((r,n)=>({type:'Feature',geometry:{type:'Point',coordinates:[r.lon,r.lat]},properties:{key:`${r.type}_${r.id}_${n}`,label:r.name||label(id),kind:r.kind||''}}));
   try{
     map.addSource('pv-poi-results',{type:'geojson',data:{type:'FeatureCollection',features}});
     map.addLayer({id:'pv-poi-points',type:'circle',source:'pv-poi-results',minzoom:10,paint:{'circle-radius':['interpolate',['linear'],['zoom'],10,6,14,8,17,10],'circle-color':'#72e8ff','circle-stroke-width':2.2,'circle-stroke-color':'#07101b','circle-opacity':1}});
     map.addLayer({id:'pv-poi-labels',type:'symbol',source:'pv-poi-results',minzoom:10,layout:{'text-field':['get','label'],'text-size':['interpolate',['linear'],['zoom'],10,11,14,12.5,18,14],'text-offset':[0,1.35],'text-anchor':'top','text-max-width':13,'text-allow-overlap':true,'text-ignore-placement':true},paint:{'text-color':'#fff','text-halo-color':'#07101b','text-halo-width':2.2,'text-halo-blur':.25,'text-opacity':1}});
   }catch(e){console.warn('Perkussus category render',e)}
 }
